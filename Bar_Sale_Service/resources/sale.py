    ...
    def create(body):
        ...
        if delivery:
            ...
            for product_info in body['product_ids']: 
                ...
                publish_message(project="adaprojects",topic="inventory_update",message=json.dumps({"id":product_info["product_id"], "amount_sold":product_info["quantity"]}),event_type="Inventory")
            session.commit()
            session.refresh(sale)
            session.close()
        publish_message(project="adaprojects",topic="balance_update",message=json.dumps({"id":body["buyer_id"], "new_balance": "10"}),event_type="Balance")
