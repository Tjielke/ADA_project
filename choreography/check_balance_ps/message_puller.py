...
class Callable:
    ...
    def callback(self, message):
        ...
        if event_type == "BalanceReq":
            balanceAva = self.product.get_quantity(data["balance"])
            if balanceAva < data["balance"]:
                data = {
                    "message": "Not enough balance"
                }
                data = json.dumps(data).encode("utf-8")
                publish_message(project=self.project, topic="balance_status", message=data,
                                event_type="NotEnoughBalance")
            else:
                data = json.dumps(data).encode("utf-8")
                publish_message(project=self.project, topic="balance_status", message=data,
                                event_type="EnoughBalance")
